...
- [x] Databricks Runtime を使用したクラスターソフトウェアのバージョン管理方法を特定する。
- [x] クラスターをフィルタ処理して、ユーザーがアクセス可能なものを表示する方法を特定する。
- [x] クラスターが終了する仕組みと、クラスターの終了が及ぼす影響を説明する。
- [x] クラスターの再起動が役に立つシナリオを特定する。
- [x] 同じノートブック内で複数の言語を使用する方法について説明する。
- [x] あるノートブックを別のノートブック内から実行する方法を特定する。
- [x] ノートブックを他人と共有する方法を特定する。
- [x] Databricks Repos が Databricks 内で CI/CD ワークフローを実現する方法について説明する。
- [x] Databricks Repos 経由で利用可能な Git の操作を特定する。
- [x] Repos との比較で Databricks ノートブックのバージョン管理機能にどのような制限があるかを特定する。
...
- [x] 結合クエリーに基づいて返される結果を特定する。
- [x] ワイド形式からロング形式にデータを変換する手段として PIVOT 句を特定する。
- [x] SQL UDF を定義する。
- [x] 関数の場所を特定する。
- [x] SQL UDF を共有するためのセキュリティモデルについて説明する。
- [x] SQL コードに CASE/WHEN を使用する。
- [x] カスタム制御フローに CASE/WHEN を活用する。

## セクション 3: 増分データ処理
- [x] Delta Lake が ACID トランザクションを提供する場所を特定する
- [x] ACID トランザクションのメリットを特定する。
- [x] トランザクションが ACID に準拠しているかどうかを特定する。
- [x] データとメタデータを比較対照する。
- [x] マネージドテーブルと外部テーブルを比較対照する。
- [x] 外部テーブルを使用するシナリオを特定する。
- [x] マネージドテーブルを作成する。
- [x] テーブルの場所を特定する。
- [x] 前のバージョンのテーブルを記述した人物を特定する。
- [x] テーブルトランザクションの履歴を確認する。
- [x] テーブルを前のバージョンにロールバックする。
- [x] テーブルを前のバージョンにロールバックできることを特定する。
- [x] 特定のバージョンのテーブルをクエリーする。
- [x] Delta Lake ファイルのディレクトリ構造を調べる。
...
- [x] 生成された列を作成する。
- [x] テーブルのコメントを追加する。
- [x] CREATE OR REPLACE TABLE と INSERT OVERWRITE を使用する
- [x] CREATE OR REPLACE TABLE と INSERT OVERWRITE を比較対照する
- [x] MERGE を使用すべきシナリオを特定する。
- [x] 保存時にデータの重複を排除するコマンドとして MERGE を特定する。
- [x] MERGE コマンドのメリットを説明する。
- [x] COPY INTO ステートメントではターゲットテーブルのデータの重複が排除されない理由を特定する。
- [x] COPY INTO を使用すべきシナリオを特定する。
- [x] COPY INTO を使用してデータを挿入する。
- [x] 新しい DLT パイプラインを作成するために必要なコンポーネントを特定する。
- [x] パイプライン作成におけるターゲットとノートブックライブラリの目的を特定する。
- [x] コストとレイテンシーの観点から、トリガー式のパイプラインと継続的なパイプラインを比較対照する
- [x] Auto Loader を利用しているソースの場所を特定する。
- [x] Auto Loader が有益なシナリオを特定する。
- [x] Auto Loader では JSON ソースから推論されたデータがすべて STRING になる理由を特定する
- [x] 制約違反のデフォルト動作を特定する
- [x] 制約違反に対する ON VIOLATION DROP ROW と ON VIOLATION FAIL UPDATE の影響を特定する
- [x] チェンジデータキャプチャと、APPLY CHANGES INTO の動作について説明する
- [x] イベントログをクエリーして、メトリクスの取得、監査ロギング、リネージの検査を行う。
...
## セクション 5: データガバナンス
- [x] データガバナンスの 4 領域のいずれかを特定する。
- [x] メタストアとカタログを比較対照する。
- [x] Unity Catalog でセキュリティの設定が可能な内容を特定する。
- [x] サービスプリンシパルを特定する。
- [x] Unity Catalog と互換性がある、クラスターのセキュリティモードを特定する。
- [x] UC が有効な All-Purpose クラスターを作成する。
- [x] DBSQL ウェアハウスを作成する。
- [x] 3 層の名前空間に対してクエリーを実行する方法を特定する。
- [x] データオブジェクトのアクセスコントロールを実装する
- [x] メタストアとワークスペースを同じ場所に配置することがベストプラクティスであると特定する。
- [x] サービスプリンシパルを接続に使用することがベストプラクティスであると特定する。
- [x] カタログ全体で事業部門を分けることがベストプラクティスであると特定する。

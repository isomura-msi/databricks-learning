...
-- MAGIC %md
-- MAGIC # Deltaテーブルの作成 (Setting Up Delta Tables)
-- MAGIC
...
-- MAGIC - テーブルスキーマを、生成された列と説明的なコメントを使って宣言する
-- MAGIC - データの場所、クォリティの強制、およびパーティショニングを制御するための詳細オプションを設定する
-- MAGIC - シャロークローンとディープクローンを生成する
...
-- MAGIC %md
-- MAGIC CTAS文は、自動的にクエリの結果からスキーマ情報を推測し、手動のスキーマ宣言をサポート**しません**。
-- MAGIC
-- MAGIC つまり、CTAS文は、明確に定義されているスキーマのソース（例えば、Parquetのファイルとテーブル）からの外部データの取り込みに役立ちます。
-- MAGIC
-- MAGIC CTAS文で、追加ファイルオプションの指定もサポートしません。
-- MAGIC
...
-- MAGIC
-- MAGIC ここでは、ユーザーを特定する情報もしくは項目別の購入詳細の情報を意図的に除外していることを前提とします。 
-- MAGIC また、ダウンストリームシステムがこのソースデータと異なる命名規則を使用している前提でフィールドの名前を変更します。
...
-- MAGIC
-- MAGIC Delta Lakeが書き込み時にスキーマを強制するため、Databricksは標準SQLの制約管理の句をサポートでき、テーブルに追加するデータの品質と整合性を確保できます。
-- MAGIC
...
-- MAGIC * **`COMMENT`** を追加し、テーブルをより簡単に見つけられるようにします
-- MAGIC * **`LOCATION`** を指定し、（マネージドテーブルではなく）外部テーブルにします。
-- MAGIC * テーブルは日付の列で **`PARTITIONED BY`** されています。つまり、各日付のデータは、指定したロケーション下の日付に対応したディレクトリに保存されます。
-- MAGIC
...
-- MAGIC
-- MAGIC **`DEEP CLONE`** は、ソーステーブルからターゲットテーブルにデータとメタデータを完全にコピーします。 このコピーはインクリメンタルに行われるので、このコマンドを再度実行するとソースからターゲットの場所に変更を同期できます。
